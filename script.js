...
function fetchData() {
  ...
  fetch(`https://ipinfo.io/${userIP}/json?token=${accessToken}`)
    ...
    .then((data) => {
      ...
      Hostname.innerHTML += `<h3>Hostname : ${data.hostname}</h3>`;
...
filter.addEventListener("input", () => {
  ...
  postLists.forEach((post) => {
    const name = post
      .querySelector(".post-office-name")
      .textContent.toLowerCase();
    const branchType = post
      .querySelector(".branch-type")
      .textContent.toLowerCase();
    const deliveryStatus = post
      .querySelector(".delivery-status")
      .textContent.toLowerCase();
    ...
    if (
      ...
      branchType.includes(filterValue) ||
      deliveryStatus.includes(filterValue) ||
      district.includes(filterValue) ||
      division.includes(filterValue)
    ) {
